...
class Simulator:
    ...
    def __init__(self, start_date, end_date, case, params=None):
        ...
        with open("action_space_cluster/clusters.pkl", "rb") as f:
            self.clusters = pickle.load(f)
        self.high_level_actions = self._infer_action_space()
    ...
    def step(self, actions):
        ...
        actions = self.gather_action(actions, self._decode_state([self.grid_states[-1]]))
        ...
        actual_consumption, actual_generation_cost, actual_production = self.apply_control(actions,
                                                                                           actual_charge,
                                                                                           actual_discharge,
                                                                                           n_storages)
    ...
    def gather_action(self, actions, state=None):
        if not isinstance(actions, GridAction):
            if self.data['action_space'].lower() == "discrete":
                actions = self._construct_action_from_list(self._construct_action_cluster(actions))
            else:
    ...
    def _infer_high_level_actions(self):
        ...
        combinations_list = itertools.product('CD', repeat=len(self.grid.storages))
    ...
    def _infer_action_space(self):
        ...
        return list(sorted(self.clusters.keys()))
    ...
    def _construct_action(self, high_level_action):
        ...
        if positive(net_generation):
            ...
            for b in storages_to_discharge:
                storage = self.grid.storages[b]
                soc = state_of_charge[b]
                discharge[b] = min(soc * storage.discharge_efficiency / d, non_flex_production,
    ...
    def _construct_action_cluster(self, high_level_action):
        action = np.random.normal(self.clusters[high_level_action]["mean"], self.clusters[high_level_action]["std"])
        return action
