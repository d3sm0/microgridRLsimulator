...
#  TODO Add constraint: when there are several storages, prevent the simultaneous charge and discharge of different
...
def _check_gurobi():
    try:
        import subprocess
        p = subprocess.Popen(['gurobi'], stderr=subprocess.PIPE)
...
class OptimizationAgent(Agent):
    ...
    def __init__(self, env):
        # control_horizon, simulation_horizon, options_filename=None, save_data=False,
        ...
        :param: options_filename: the agents options filename.
        ...
        with open('agents/agent_options/Optimization/agent_options_exact.json', 'r') as fin:
            params = json.load(fin)['Optimization']
    ...
    def train_agent(self):
        pass  # Nothing to train the Optimization agents with
        ...
        for i in range(1, self.n_test_episodes + 1):
            ...
            while not done:
                ...
                for j in range(len(low_level_actions)):
                    ...
                    # Run the simulator in continuous mode (low_level_action)
                    next_state, reward, done, info = self.env.step(actions=low_level_actions[j])
    ...
    def get_optimal_action(self):
        ...
        for p in self.model.SimulationPeriods:
            ...
            GridActions.append(GridAction(charge, discharge, generation))
        return GridActions
